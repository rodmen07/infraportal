/**
 * Patch notes data for PatchNotesPage.tsx, extracted (v1.17.4) so the release
 * history is plain structured data: unit-testable in Vitest and importable
 * without rendering, while the page keeps fast-refresh-friendly
 * component-only exports.
 *
 * Ordering contract: newest first, and every group's entries are contiguous
 * (the page injects one group header before the first card of each group).
 * patchNotesData.test.ts enforces the sanity rules.
 */

export type Severity = 'high' | 'medium-high' | 'medium' | 'low-medium'
export type CompletionState = 'planned' | 'implemented' | 'published'
...
export const GROUP_META: Record<string, { label: string; status: string }> = {
  'v1.17': { label: 'Interactive API Playground', status: 'Complete' },
  'v1.16': { label: 'Developer Experience, API Surface & Portal UX', status: 'Complete' },
...
export const VERSIONS: Version[] = [
  {
    tag: 'v1.17.4',
    date: '2026-07-19',
    label: 'Cross-Repo Spec Drift Detection & v1.17 Wrap-up',
    completionState: 'published',
    group: 'v1.17',
    summary:
      'Closes the Interactive API Playground theme with cross-repo spec drift detection: a checker regenerates the OpenAPI JSON snapshots through the same deterministic pipeline as npm run sync-specs and byte-compares them against the committed src/api-specs/ files, and a CI workflow runs it on spec-touching PRs plus a weekly schedule so drift arriving in the microservices repo without a PR here still gets noticed. Also backfills patch notes for v1.16 and v1.17 and marks the playground milestones shipped in the roadmap.',
    highlights: [
      {
        heading: 'Drift detection',
        items: [
          'scripts/check-spec-drift.mjs regenerates all 11 spec snapshots plus the manifest into a temp directory and byte-compares against src/api-specs/ over git-normalized LF content (so a Windows autocrlf checkout is not "drift"), printing a per-file report (match, drift, missing, stale) and exiting nonzero on any difference, with the documented fix: rerun npm run sync-specs and commit the diff.',
          'Sync and drift checks share one module (scripts/lib/specSnapshot.mjs), so the checker exercises exactly the pipeline the committed snapshots were built with.',
          'Local runs read the sibling microservices checkout; CI runners have no sibling checkout, so remote mode fetches the canonical specs from the public microservices repo (raw.githubusercontent.com) at a pinned ref, main by default.',
        ],
      },
      {
        heading: 'CI',
        items: [
          'New spec-drift workflow: pull requests touching src/api-specs/ or the sync scripts, a weekly Monday cron for cross-repo drift, and workflow_dispatch. Path filters keep it away from unrelated PRs; the committed snapshot stays authoritative for the site.',
        ],
      },
      {
        heading: 'Tests and docs',
        items: [
          'Vitest coverage of the snapshot build (determinism, validation, operation counts) and the directory compare (fixture dirs for clean, drifted, missing, and stale cases; report shape pinned).',
          'Patch notes data extracted to src/pages/patchNotesData.ts with sanity tests (unique tags, valid dates, group metadata present, groups contiguous); ROADMAP.md marks v1.17.1 through v1.17.4 shipped.',
        ],
      },
    ],
  },
  {
    tag: 'v1.17.3',
    date: '2026-07-19',
    label: 'API Playground: Snippets & Deep Links',
    completionState: 'published',
    group: 'v1.17',
    summary:
      'Adds per-operation copyable snippets and sharable deep links to the API playground. Snippet generation is template-driven and unit tested; deep links use the hash-router query form #/api-docs?service=...&op=... so GitHub Pages serves the same index.html for every link.',
    highlights: [
      {
        heading: 'Snippets',
        items: [
          'curl snippets carry the documented method, the spec\'s first documented server URL, a bearer placeholder when the operation requires auth, and a POSIX-quoted JSON body that prefers the visitor\'s current Try it form values before falling back to the spec example or a schema-derived skeleton.',
          'TypeScript SDK snippets use the real @rodmen07/infraportal-sdk surface: the typed AccountsApi for accounts CRUD, the client core\'s generic request() elsewhere with an honest note that the typed module does not exist yet; a test pins the SDK\'s public exports so no snippet references anything the SDK does not export. Labeled builds-from-source, not yet on npm.',
          'An offline note states the base URL is the historical Cloud Run deployment, offline since the 2026-06-04 decommission; a PowerShell note covers curl.exe and quoting differences.',
        ],
      },
      {
        heading: 'Deep links',
        items: [
          'Pure parse/serialize helpers (parseApiDocsHash, buildApiDocsHash) round-trip under test; a cold load on a deep link restores the selected service and operation.',
          'Per-operation Copy link affordance so specific endpoints can be linked from case studies and outreach.',
        ],
      },
    ],
  },
  {
    tag: 'v1.17.2',
    date: '2026-07-19',
    label: 'API Playground: Try It Against the Demo Stores',
    completionState: 'published',
    group: 'v1.17',
    summary:
      'Adds a per-operation Try it panel that executes requests against the in-browser demo stores through an explicitly marked mock boundary (src/lib/tryItAdapter.mock.ts). 28 of the platform\'s 99 documented operations execute against demo data (accounts, contacts, opportunities via crmStore.mock.ts; projects, milestones, deliverables via projectsStore.mock.ts); every other operation renders an honest disabled state instead of faking liveness.',
    highlights: [
      {
        heading: 'Request builder',
        items: [
          'Forms derive from each operation\'s parameters and requestBody schema, with a JSON textarea fallback and a Form/JSON toggle so every documented error path stays reachable; demo-id hint chips fill path params and the panel states the admin-caller simulation outright.',
        ],
      },
      {
        heading: 'Spec-accurate simulation',
        items: [
          '200/201/204 DTOs come from real store state; VALIDATION_ERROR envelopes follow each spec\'s exact status split (400 for accounts/contacts/projects, 422 for opportunities); 404s come from store misses.',
          'Contacts\' cross-entity 422 INVALID_ACCOUNT is checked against the demo accounts; the projects-service no-cascade delete failure is reproduced as 500 DB_ERROR; axum extractor rejections (malformed JSON 400, missing field or wrong type 422) return text/plain as documented.',
          'X-RateLimit-* headers appear only where each response documents them, tagged simulated.',
        ],
      },
      {
        heading: 'Tests',
        items: [
          '55 new Vitest cases: adapter operation mapping, form-model derivation, store CRUD, disabled states, and a per-operation panel render smoke. The builder ships inside the lazy api-docs chunk.',
        ],
      },
    ],
  },
  {
    tag: 'v1.17.1',
    date: '2026-07-19',
    label: 'API Playground: Committed Specs Rendered On-Site',
    completionState: 'published',
    group: 'v1.17',
    summary:
      'Renders all 11 committed OpenAPI 3.0.3 specs on the deployed site with a custom lightweight client-side renderer and zero runtime network requests, and restores the API Docs nav link after the funnel entries. npm run sync-specs converts the specs from the microservices repo into deterministic JSON snapshots committed under src/api-specs/.',
    highlights: [
      {
        heading: 'Spec snapshot pipeline',
        items: [
          'Deterministic sync script (exact-pinned yaml devDependency, stable service order, 2-space indent, no timestamps) plus a generated manifest that drives the service selector and every derived count on the page.',
          'Specs lazy-load as one Vite chunk per service and the route itself is lazy, so the initial bundle shrinks; no YAML parser ships in the browser.',
        ],
      },
      {
        heading: 'Renderer',
        items: [
          'Operations grouped by tag with parameters, request/response schema trees, local $ref resolution with cycle guards, per-operation auth requirements, and ApiError envelope tagging.',
          'Dead GATEWAY_URL links and the hardcoded services table deleted; page copy derives from the manifest instead of hand-maintained claims.',
        ],
      },
      {
        heading: 'Tests',
        items: [
          'Spec-model unit tests plus a walk-every-operation catalog suite: all $refs resolve, the manifest matches the specs, and every operation render-smokes without a fallback path.',
        ],
      },
    ],
  },
  {
    tag: 'v1.16',
    date: '2026-07-19',
    label: 'Developer Experience, API Surface & Portal UX (v1.16.0-v1.16.5)',
    completionState: 'published',
    group: 'v1.16',
    summary:
      'Consolidated v1.16 release, delivered 2026-07-18 to 2026-07-19 across the microservices repo (PRs #89-#97), go-gateway (#10), and this site (#20-#22). The developer-experience theme gave the 11-service platform a complete documented API surface: OpenAPI specs for every service, API and rate-limit guides, a gateway spec aggregator, a TypeScript SDK, and a Postman collection. The portal UX theme shipped bulk operations and templates against explicitly marked in-browser demo stores. Cost Intelligence, the old middle theme, stays blocked by the 2026-06-04 decommission and shipped nothing.',
    highlights: [
      {
        heading: 'v1.16.0 Auth hardening and roadmap reset',
        items: [
          'shared-auth crate and the published axum-api-kit crate adopted across the workspace; role-based authorization gating extended to all workspace services with integration tests.',
          'Committed ROADMAP.md recording the 2026-06-04 decommission and the doable-now / blocked / user-only partition.',
        ],
      },
      {
        heading: 'v1.16.1-.3 Developer experience (microservices repo)',
        items: [
          'OpenAPI 3.0.3 specs for all 11 workspace services, linted in CI with Redocly CLI pinned to 1.25.15; docs/API.md (auth model, 401/403 semantics, error envelope, rate-limit tiers, spec index) and docs/RATE_LIMITING.md.',
          'go-gateway /api/openapi.json spec aggregator route with unit tests.',
          'TypeScript SDK at sdks/typescript-sdk/: InfraPortalClient core (retry contract, rate-limit parsing, error envelope), generated types for all 11 services, and a typed accounts module; build-only, npm publish stays user-only. Postman collection in postman/.',
        ],
      },
      {

...
// ---------------------------------------------------------------------------
// Portal login gate — shown when no valid bearer token is available
// ---------------------------------------------------------------------------
export function PortalLoginGate() {
  const callback = encodeURIComponent(
    `${window.location.origin}${window.location.pathname}?#/crm/admin`
  )
  ...
  return (
    <div className="flex min-h-[50vh] items-center justify-center px-4">
      <div className="forge-panel surface-card-strong w-full max-w-sm rounded-3xl p-8 shadow-2xl shadow-black/50 space-y-6">
        <div>
          <h2 className="text-base font-semibold text-text-primary">Sign in required</h2>
          <p className="mt-1 text-xs text-text-muted">
            Log in with your portal account to access live CRM data.
          </p>
        </div>
        {AUTH_SERVICE_URL ? (

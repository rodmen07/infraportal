...
- **PR 2:** typography and contrast: apply the named type scale per D5, floor interface text at 12px (replacing the 163 arbitrary 10-11px uses where they carry content; a small allowlist may keep decorative kickers), fix the documented zinc-500-on-dark contrast failure, and delete the last of the light-mode override sheet. Add a grep-style Vitest gate: no new `text-[10px]`/`text-[11px]` outside the allowlist, no `[data-theme="light"] .` utility-keyed overrides.
- **Done when:** primitives exist with unit tests and the named pages use them; override sheet deleted; axe smoke and the new gates green; both-theme walk clean; deploy green.
- **Proves to the audience:** tables, badges, and buttons look related everywhere; light mode becomes a first-class theme instead of a patch; reviewers reading the repo find a `ui/` layer instead of 2,000-line pages defining their own pills.
...
- **The override sheet is deleted too early.** Mitigation: deletion is an exit criterion of v1.18.4, never a starting move; until then the sheet and the tokens coexist.
...
- **Cadence.** Four required minors plus one optional fits the one-minor-per-week cadence with the theme complete by mid-August 2026.
...
Unchanged from the roadmap, applied per milestone: `npm run build`, `npm run test` (Vitest), the tsc CI gate, npm audit CI, and a green GitHub Pages deploy on merge, plus the milestone-specific gates listed in Section 5 (token integrity test, axe smoke, typography gate, chunk budget).

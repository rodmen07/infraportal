...
### v1.18 - One Product: UI/UX coherence theme - IN PROGRESS. Design doc: `docs/design/V1_18_UX_THEME.md`, APPROVED 2026-07-19 (user replied "defaults approved" to all eleven decisions D1-D11; recorded in PR #30).

- **v1.18.1 PR1 (shipped 2026-07-19, PR #31).** Token foundation: `src/styles/tokens.css` defines semantic roles for both themes (surface, border, text, accent, status, radius, spacing, type scale), mapped into `tailwind.config.js`. Fixes audit finding F1 by giving `forge-panel`, `forge-grid`, `surface-card`, `surface-card-strong` and `interactive-card` real definitions in both themes, so the cards that rendered invisible in dark mode are gone by construction rather than by hunting call sites. Fixes F3, the first-load theme flash, via one shared `DEFAULT_THEME` constant that index.html and ThemeProvider now agree on. Tests 337 to 383.
- **v1.18.1 PR2 (shipped 2026-07-20, PR #32).** Shared chrome onto tokens: TopNav, SideNav, PageLayout, SlideOver, the button and chip recipes, and a new shared `.field-input` replacing three hand-copied form recipes. Seven override-sheet rule groups deleted under the strangler rule, including both duplicate `.interactive-card` blocks and the entire `--fx-*` variable set (mechanism 3 of F2). The token integrity test now scans all 91 `src/**/*.tsx` and caught a ghost the first pass missed: `.btn-sm`, used 18 times and defined nowhere. Five real light-mode defects fixed en route (nav pills, header note, Try-it inputs, disabled button colours, and the documented zinc-500 contrast failure). Tests 383 to 504.
- **v1.18.2 (shipped 2026-07-20, PR #34 + PR #35).** One nav (SideNav retired, TopNav everywhere), one brand ("RM Cloud Consulting" unified), one page anatomy (FocusCard's full-viewport pattern retired, 9 pages onto a shared `PageHeader`), plus real focus-trap/Escape/skip-link a11y for SlideOver and the mobile drawer.
- **v1.18.3 (shipped 2026-07-20, PR #36; QA follow-up PR #37).** Calm hero motion (D6), canonical "free 30-minute discovery call" copy (D8), shortened 4-field contact form (D9), a new ProofStrip on Home, OG/Twitter meta tags. PR #37 fixed 2 contrast regressions and 2 voice-test gaps found in adversarial QA of #36.
- **v1.18.4 (shipped 2026-07-20, PR #38 + PR #40, hotfix PR #41).** Shared UI primitives (`Badge`, `Button`, `Card`, `DataTable`, `EmptyState`) absorbing ad hoc pill/CTA palettes from PatchNotes/Consultations/SupportQueue/Portal/PricingCard (CrmAdminPage explicitly excluded, per its own scope fence); D5 type scale (16px root, 12px interface floor); the two named repo-wide contrast bugs (`border-amber-400/30`/`/60`, `bg-emerald-500/10`) fixed for every real consumer including `hover:`/`focus:`/`open:` variants; F11's dark zinc-500 contrast failure fixed. **Override-sheet deletion did NOT happen this milestone** - the sheet grew from 124 to 229 `[data-theme="light"]` selectors (closing more real contrast gaps required more overrides, not fewer, while the ~40 files still on raw Tailwind utilities remain unmigrated) - so full deletion is retargeted to the new v1.18.6 milestone below; a ratchet gate keeps the sheet from growing further unnoticed. PR #40 recreates PR #39 (GitHub auto-closed #39 when #38's base branch was deleted on merge and could not be retargeted on a closed PR) with the same commits, targeting `main` directly. Hotfix PR #41 fixed a build-breaking regression both merges introduced: several new CSS comments described Tailwind wildcard prose (`bg-*/border-*`, `src/**/*.tsx`) containing a literal `*/`, which is the CSS comment-close token, so PostCSS choked on the resulting invalid CSS - not caught by any required check since none of them run `npm run build`.
- **v1.18.6 - Override-sheet retirement (RE-SCOPED 2026-07-22 after a measured analysis; user-approved HYBRID plan, not yet started).** A verified boundary-matched measurement of `main` found the sheet is ~230 `[data-theme="light"]` rules, ~179 keyed to raw palette classes. Of those: 4 already dead, 97 behind ≤3 files, 18 behind 4-10, 4 hotspots (11+), and **56 blocked by the code-health-fenced `CrmAdminPage.tsx`/`PortalPage.tsx`**. So full deletion to zero is NOT achievable here. **Re-scoped done-when:** retire every non-fence-blocked palette rule (~123), leaving the sheet at its ~105-rule structural floor (≈49 structural + 56 fenced). **Approach:** PR1 = a curated codemod of only the ~14-23 pixel-clean classes (a blanket codemod is unsafe: ~73% of classes would shift a colour, and a name-based map mis-buckets); then class-oriented per-PR for the easy/medium consolidation classes with a manual both-theme walk; hotspots get their own PRs. A `build` job is added to required checks first (the `*/` PostCSS hazard is un-gated). Full detail: `docs/design/V1_18_UX_THEME.md` section 5.
- **CH-1 - Split CrmAdminPage / PortalPage, then retire the last 56 override rules (dependent code-health milestone, filed 2026-07-22).** Behavior-preserving split of the two fenced pages (2,187 + ~1,100 lines), migrate the extracted pieces onto tokens, then delete the final 56 rules to reach the sheet's true zero. Sequenced after v1.18.6; a v1.19 code-health candidate.
- Remaining in the v1.18 theme: **v1.18.6** (override-sheet retirement, re-scoped above). v1.18.5 (route-level code splitting) SHIPPED 2026-07-21 as PR #44 (initial chunk 800.54 kB → 387.92 kB, −51.5%).

...
- **v1.18.6 - Override-sheet retirement (added post-hoc, not yet started).** Migrate the ~40 case-study/admin/portal files still on raw dark-mode Tailwind utilities onto the `surface-*`/`text-*`/`border-*`/`accent-*` tokens, the actual override-sheet-deletion exit criterion v1.18.4 could not carry. See `docs/design/V1_18_UX_THEME.md` section 5.
- Remaining: v1.18.5 (optional, route-level code splitting / chunk budget) and v1.18.6 (override-sheet retirement) are the only items left in the v1.18 theme.
